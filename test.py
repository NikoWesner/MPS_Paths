...
def permuter(expo):
    ...
    return permuter
def swap_permutation_matrix_MPO(n, i, j):
    """
    Create an n x n permutation matrix that swaps entries i and j.
    
    Parameters:
    n : int - size of the vector
    i, j : int - indices to swap (0-based)
    
    Returns:
    P : np.ndarray - permutation matrix
    """
    P = np.eye(n)  # Start with identity matrix
    if i == j:
        return P
    # Swap rows i and j
    P[[i, j]] = P[[j, i]]
    return P
def Permute_MPO(expo,i,j):
    A=swap_permutation_matrix_MPO(2**expo,i,j)
    MPO1=MPO(expo)
    MPO_Size=2*np.ones(2*expo,dtype=int)
    A=np.reshape(A,MPO_Size)
    MPO1.truncated_r(A,1E-6)
    return MPO1
...
def randomfunction(x,y):
    o=np.ones_like(x)
    l=np.shape(o)
    o[0:l[0]//2,0:l[1]//2]=np.zeros((l[0]//2,l[1]//2))
    return o
...
path=r"C:\VS\Python\MPS\MPS_Paths\data\2D_VortexFlow_ETH\0500.am"
...
MPS1,r1,s1=XYMPS(U0,expo,e)
MPSG,rg,sg=GourianovMPS(U0,expo,e)
MPSH,rh,sh=HilbertMPS(U0,expo,e)
MPS2,r2,s2,inv,d1=minmaxMPS(U0,expo,e)
print(f"XY:{r1},{s1}")
print(f"Gourianov:{rg},{sg}")
print(f"Hilbert:{rh},{sh}")
print(f"minmax:{r2},{s2}")
# getGouianovplots(MPS1,MPS2,MPSG,MPSH,U0,expo)
...
MPS1,r1,s1=XYMPS(fun2,expo,e)
MPSG,rg,sg=GourianovMPS(fun2,expo,e)
MPSH,rh,sh=HilbertMPS(fun2,expo,e)
MPS2,r2,s2,inv,d1=minmaxMPS(fun2,expo,e)
print(f"XY:{r1},{s1}")
print(f"Gourianov:{rg},{sg}")
print(f"Hilbert:{rh},{sh}")
print(f"minmax:{r2},{s2}")
# getGouianovplots(MPS1,MPS2,MPSG,MPSH,fun2,expo)
print(f"Lineardependency:{d1}")
